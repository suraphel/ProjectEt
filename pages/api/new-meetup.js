...
async function Handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    "mongodb+srv://admin:MONGODB@cluster0.xdax3.mongodb.net/chores?retryWrites=true&w=majority"();
    const db = client.db(); // to connect to the client-side db
    const chorecollection = db.collection("chores"); // create a collection called meetup
    // result is an object with an id as one of its properties
    const result = await chorecollection.insertOne(data); // in Mongodb doc is an js object
